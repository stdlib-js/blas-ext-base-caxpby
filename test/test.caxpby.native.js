...
var caxpby = tryRequire( resolve( __dirname, './../lib/caxpby.native.js' ) );
var opts = {
	'skip': ( caxpby instanceof Error )
};
...
tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( caxpby.length, 7, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the output array', opts, function test( t ) {
	...
	x = new Complex64Array( [ 1.0, 2.0, 3.0, -1.0, 0.0, 1.0 ] );
	y = new Complex64Array( [ 2.0, 1.0, -1.0, 3.0, 4.0, 0.0 ] );
	out = caxpby( x.length, alpha, x, 1, beta, y, 1 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `y` unchanged', opts, function test( t ) {
	...
	caxpby( 0, alpha, x, 1, beta, y, 1 );
	t.strictEqual( isSameComplex64Array( y, expected ), true, 'returns expected value' );

	caxpby( -4, alpha, x, 1, beta, y, 1 );
	t.strictEqual( isSameComplex64Array( y, expected ), true, 'returns expected value' );

	t.end();
});

tape( 'when `alpha` equals `0`, the function scales `y` by `beta`', opts, function test( t ) {
	var expected;
	var alpha;
	var beta;
	var x;
...
tape( 'when `beta` equals `1`, the function multiplies `x` by a constant and adds the result to `y`', opts, function test( t ) {
	var expected;
	var alpha;
	var beta;
	var x;
